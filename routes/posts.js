...
router.get('/:post_id', function (req, res, next) {
    ...
    if (category === 'community') {
        ...
        function selectFurniture(i_details, connection, callback) {
            var furnituresql = "SELECT f.f_photo_path, f.brand, f.name, f.no, f.size, c.code, f.link, f.price " +
...
router.put('/:post_id', isLoggedIn, function (req, res, next) {
    ...
    form.parse(req, function (err, fields, files) {
        ...
        if (!files['photo']) {  // content만 고쳤을 때

            function updateContent(connection, callback) {
                ...
                connection.query(contentsql, [content, postId, user.id], function (err) {
                    if (err) {
                        connection.release();
                        err.code = "E00004";
                        err.message = "게시물 내용을 수정할 수 없습니다.";
                        callback(err);
                    } else {
                        callback(null, connection);
                    }
            ...
            function updateTag(connection, callback) {
                var hash_tag = formFields['tag'];

                var deleteTag = "DELETE FROM hashtag_has_post "+
                                "WHERE post_id=?";
                connection.query(deleteTag, [postId], function (err) {
                    if (err) {
                        err.code = "E00004";
            ...
            async.waterfall([getConnection, updateContent, updateTag], function (err) {
                res.json({result: {message: "커뮤니티 게시글이 수정되었습니다(사진업데이트 안할 시)"}});
            })

        } else {  // photo와 content모두 수정
            var location = "";
            ...
            async.waterfall([getConnection, deletePhoto, deleteTags, upDatePhoto], function (err) {
                if (err) {
                    next(err);
                } else {
                    res.json('커뮤니티 게시글 변경이 완료되었습니다(사진 업데이트 시)');
                }
            }); // async.waterfall
        }
    });   // form.parse
